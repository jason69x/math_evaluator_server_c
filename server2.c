/*  SERVER 2 - concurrent clinets using fork() 
 *  */
...
int main(int argc,char *argv[]){
  ...
  while(1){
    ...
    // create a child process and interact with client on child process, while listening for other clients
    if(!fork()){ // using fork to create a child process that handles the client
      pid_t child_pid = getpid();
    int bytes_recv; char recv_buf[MAX_BUF_SIZE];
    while( (bytes_recv = recv(new_fd,recv_buf,MAX_BUF_SIZE-1,0))!=0){
      if(bytes_recv == -1){
        fprintf(stderr, "server [%d]: failed to receive\n",child_pid);
        close(new_fd);
        continue;
      }
      recv_buf[bytes_recv] = '\0';
      printf("server [%d]: received from client %s\n",child_pid,recv_buf);
    ...
    printf("server [%d]: closed connection with client %s:%d\n",child_pid,client_ip,ntohs(client_addr.sin_port));
    close(new_fd);
